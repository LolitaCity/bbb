...
class Capital extends CI_Controller {
    ...
    { //导出
        ...
        $outbanknumber = $this->blank->getOne($id);
        if ($outbanknumber == null) {
            echo "<script>alert('您还没有绑定转账的银行卡，请先绑定好转账的银行卡号！');history.back();</script>";
        }else{
            if (count($list) != 0) {
                ...
                $reader->setActiveSheetIndex(0)
                    ->setCellValue('A1', '收款账户列')
                    ->setCellValue('B1', '收款户名列')
                    ->setCellValue('C1', '转账金额列')
                    ->setCellValue('D1', '备注列')
                    ->setCellValue('E1', '收款银行列')
                    ->setCellValue('F1', '收款银行支行列')
                    ->setCellValue('G1', '收款省/直辖市列')
                    ->setCellValue('H1', '收款市县列');
                ...
                foreach ($list as $res) {
                    ...
                    $reader->getActiveSheet(0)->setCellValue('A' . ($i + 2), ' ' . $bankaccount);
                    $reader->getActiveSheet(0)->setCellValue('B' . ($i + 2), ' ' . $res->truename);
                    $reader->getActiveSheet(0)->setCellValue('C' . ($i + 2), ' ' . sprintf("%.2f", $res->money));
                    $reader->getActiveSheet(0)->setCellValue('D' . ($i + 2), '订单号：' . $res->ordersn);
                    $reader->getActiveSheet(0)->setCellValue('E' . ($i + 2), $bankname);
                    $reader->getActiveSheet(0)->setCellValue('F' . ($i + 2), $subbranch);
                    $reader->getActiveSheet(0)->setCellValue('G' . ($i + 2), '广东');
                    $reader->getActiveSheet(0)->setCellValue('H' . ($i + 2), '广州');;
                    $i++;
                }
