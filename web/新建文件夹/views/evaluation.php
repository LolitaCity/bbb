            ...
            <div class="tab1" id="tab1">
                ...
                <div class="menu">
                    <ul>
                        ...
                        <li id="one0"class="off" onclick="location.href='<?=site_url('sales/evaluation')?>'">评价管理</li>
                    </ul>
                </div>
                <div class="menudiv">
                    <div id="con_one_2">
                        <div class="fpgl-ss">
                            <p>
                                ...
                                <select class="select_215" id="selSearch" name="selSearch" style="width:120px;">
                                    ...
                                    <option value="rodersn">订单编号</option>
                            ...
                            <?php if($countall >0):?>
                            <p><input class="input-butto100-xxshs" style="width:100px" type="button" onclick="passAll()" value="一键审核(<?=$countall?>)" id="btnAuditAll"></p>
                            <?php endif;?>
                        </div>
                        <script>
                        function passAll(){
                            ...
                            <table>
                                ...
                                <?php foreach($list as $vl):?>
                                        <tr>
                                            <td>
                                                <p class="fpgl-td-rw">
                                                    任务编号：<?php foreach($usertask as $v){ if($v->id == $vl->usertaskid){echo $v->tasksn;}}?> </p>
                                                    <p class="fpgl-td-rw">
                                                        订单编号：<?php foreach($usertask as $v){ if($v->id == $vl->usertaskid){echo $v->ordersn;}}?></p>
                                                    <p class="fpgl-td-rw">     
                                                     <?php foreach($usertask as $v){ if($v->id == $vl->usertaskid){ ECHO substr($v->expressnum,0,strrpos($v->expressnum,'&')).'：'.str_replace('&','',strstr($v->expressnum,"&"));}}?></p>
                                            </td>
                                            <td class="fpgl-td-zs">
                                                    <p class="fpgl-td-rw">
                                                        买号：<?php foreach($wangwangs as $wws){if($wws->userid==$vl->doid)echo $wws->wangwang;}?></p>
                                                    <p class="fpgl-td-rw">
                                                        信誉值：<?php 
                                                        foreach($buyers as $by){
                                                            if($by->userid==$vl->doid){
                                                                $info= unserialize($by->account_info);
                                                                echo $info['xy_val'];
                                                               // var_dump($arrinfo);
                                                            }
                                                        }?>
                                                ...
                                                <p class="fpgl-td-rw">
                                                    商品简称：<?php                      foreach($usertask as $vutb){
                                                        if($vutb->id==$vl->usertaskid){
                                            ...
                                            <td class="fpgl-td-zs">
                                                ...
                                                <td>
                                                    <?php if($vl->doing == 0):?>
                                                        <p class="fpgl-td-mtop"><input class="input-butto100-xls" type="button" value="取消评价任务"  onclick="CancelEvaluate('<?=$vl->id?>')" style=" cursor: text;background: red;"></p>
