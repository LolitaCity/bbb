...
<body style="background: #fff;">
    ...
    $(function () {
        ...
        function Submit()
        {
            ...
            if (msg != "") {
//              $.openAlter(msg, "提示");
                return dialog.error(msg);
            }else{
//              $("#fm").submit();
				public.ajax('<?=site_url('capital/editBankDB')?>', $('#fm').serialize(), function(datas){
					if(datas.status)
					{
						dialog.success(datas.message);
						var $parent = window.parent.document; 
						console.log($parent.getElementById('tail').html);
						dialog.iframe_close();
						$parent.getElementById('tail').innerText = CardNumber.slice(-4);
						$parent.getElementById('truename').innerText = AccountName;
						$parent.getElementById('bankname').innerText = BankName;
					}
					else
					{
    ...
    <!--列表 -->
<form action="javascript:;" id="fm" method="post">  
    <input type="hidden" name='id' value="<?=$info->id?>">      
    <input type="hidden" name='bankid' value="<?=$userbank->id?>">      
    <div class="tc_yhk">  
            <ul class="tx_txt">
                <li><label>原转账银行卡：</label><?=$userbank->bankName?> 尾号 <?=substr($userbank->bankAccount,-4)?> <?=$userbank->bankAccount?></li>
                <li><label>开户人：</label><?=$userbank->truename?></li>
            </ul>
            ...
            <ul class="tx_ul">
                ...
                <li>
                    <label>银行卡开户人：</label>
                    <input type="text" maxlength="5" onkeyup="value=value.replace(/[0-9]/g, '')" onpaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\u4E00-\u9FA5]/g,''))" name="AccountName" id="txtAccountName" class="input_228" placeholder="请输入正确的开户人姓名" >
                </li>
                <li>
                    <label> 支付密码：</label>
