...
?>    
    ...
    var wainingMsg="<div style='text-align:left'>为了控制货款资金风险，平台<em style='color:red'>不允许发布</em>货款金额<em style='color:red'>超过2000元</em>的任务。对于高成交金额的商品，若出现恶意退款，平台会协助追回，但<em  style=\"text-decoration:underline\">不承担赔付责任</em>。请知悉！</div>";
    $(function () {
         $("input[date!=SingleProductPrice][date!=BuyProductCount],select,#imgReduce,#imgAdd").live("click onpaste input",function(){
          var singlePriceNum=0;
           $('input[date=SingleProductPrice]').each(function () {
                ...
                if(parseFloat(priceValue * pNum).toFixed(2)>=2000)
               {
                 singlePriceNum++;
               }
          });
        ...
        $("input[date=SingleProductPrice]").live("keyup blur", function () {
            ...
            if (priceValue > 0) {
              ...
              if(parseFloat($(this).val() * pNum).toFixed(2)>=2000)
              {
                $.openAlter(wainingMsg,"提示",null,null,"好的");
              }
        });
        ...
        $("input[date=BuyProductCount]").live("keyup blur", function () {
            ...
            $("#inputcomm"+$(this).parent().parent().attr('id').replace('option','')).val(GetPricePoint(priceValue))
             ...
             if(parseFloat(sngleProductPrice * pNum).toFixed(2)>=2000)
              {
                $.openAlter(wainingMsg,"提示",null,null,"好的");
              }
        });
...
//             $("select").removeClass("select1");
    ...
    {  
        ...
        var type='精刷单';
        var shopPoint='2';
        var isGetPrinPrice='true';
        if(isGetPrinPrice!="true")
        {
            if(type=="推送单")
            {
                point=parseFloat(point)+parseFloat(shopPoint)
            }
        }
        return point;
    }
        ...
        function submitNext() {
         ...
         $('input[date=SingleProductPrice]').each(function () {
               ...
               if(parseFloat(parseFloat(priceValue * pNum)+parseFloat(expressValue)).toFixed(2)>=1000)
               {
                 singleProductCount1++;
               }
          });
         ...
         $('input[date=TaskPlanCount]').each(function () {
                var num = $(this).val();
                var count= $(this).parent().parent().parent().prev().find("span[id=spCount]").text(); //剩余发布数量
...
//                                  }
        ...
        if (singleProductCount1 > 0) {
             $.openNewConfirm("<div style='text-align:left'>从<em style='color:red'>6月9日</em>起，对于真实成交价格<em style='color:red'>超过1000元</em>的订单，平台不承担恶意退款的赔付责任，请知悉并慎重考虑资金风险！</div>", "提示", { width: 250, height: 50 }, function () {   $('#frm').submit();  }, "我知道了", function () { $.self.parent.$.closeAlert() }, "返回修改");
                return false;
            }
          $('#frm').submit(); //提交表单
        }
                    ...
                    <div style="width: 500px; margin-top: 5px;">
                        <div class="fprw-pg">
                            <table id="optionContainer2">
                                <tbody><tr>
                                    <th width="231">开始时间</th>
                                    <th width="231">结束时间</th>
                                    <th width="231">超时时间 (选填)</th>
                                </tr>
                                <tr>
                                    <td>
        ...
        <div class="sj-fprw">
            ...
            <div class="fprw-pg" style="margin-top: 0px;">
                <table id="optionContainer">
                    ...
                    <tr id="option0">
                        <td>
                            <input type="text" name="SingleProductPrice[]" date="SingleProductPrice" class="input_60" value="0" placeholder="0" maxlength="5" onkeyup="clearNoNum(this)">
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="text" name="BuyProductCount[]" date="BuyProductCount" value="1" placeholder="1" onkeyup="value=value.replace(/[^0-9]/g,'')" class="input_60">
                        </td>
                        ...
                        <tr>
                            ...
                            <td style="padding-top: 0px; padding-bottom: 0px;">
                                <div style="height: 24px; margin-top: 8px; margin-left: 60px;">
                                    ...
                                    <p class="left" style="margin-top: 2px"><input type="text" name="TaskPlanCount[<?=$n?>]" date="TaskPlanCount" value="<?=$n==0?$all:0?>" placeholder="0" onkeyup="value=value.replace(/[^0-9]/g,'')" class="input_40" disabled="disabled"></p>
