			    ...
			    <div class="layui-timeline-content layui-text">
			      ...
			      <div>
			      	<div class="layui-form-item">
					    ...
					    <div class="layui-input-inline" style="width: 17%;">
					      <input type="text" name="number" required  lay-verify="required|number" placeholder="请输入数量" autocomplete="off" class="layui-input about_money about_top">
					    </div>
				  	</div>
			  ...
			  <li class="layui-timeline-item">
			    ...
			    <div class="layui-timeline-content layui-text">
			      ...
			      <div class="layui-form-item">
				    ...
				    <div class="layui-input-inline" style="width: 17%;">
				      <input type="text" AUTOCOMPLETE="off" name="top" oninput="clearNoNum(this)"  placeholder="请输入置顶费(元/单)" autocomplete="off" class="layui-input about_top">
				    </div>
				    <div class="layui-form-mid layui-word-aux"><span>元/单</span>&nbsp;&nbsp;&nbsp;&nbsp;<span id="compute_top"></span></div>
			  	  </div>
	...
	<script type="text/javascript">
		...
		$('.about_top').bind('input', function(){
			...
			if ($top > 50)
			{
				...
				dialog.error('亲，平台限制置顶费用的最高金额为50元。您设置的金额数已超过50，请进行修改');
			}
			else
				$('#compute_top').html($number + '单 * ' + $top + '元/单 = ' + $count + '元');
		})
		  			...
		  			if (data.field.gettime == 0 || $average_time[0].trim() < $average_time[1].trim())
			  		{
			  			...
			  			if (data.field.close.trim() >= $now)
			  			{
			  				if (data.field.gettime == 0 || data.field.close.trim() >= $average_time[1].trim())
				  			{
				  				...
				  				data.field.action_commission = $('span#commission_all').html();
				  				data.field.top_count = parseInt($('input[name=top]').val()) * parseInt($('input[name=number]').val());
				  				data.field.ShopID = <?=$sid;?>;
				  				layer.prompt({
								  formType: 1,
								  title: '请输入安全码以完成发布',
	    ...
	    function clearNoNum(obj) 
		{
		    ...
		    if (!obj.value)
		    {
		    	obj.value = obj.name == 'express' || obj.name == 'top' ? 0 : 1;
		    }
		}
