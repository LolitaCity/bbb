...
<form action="<?=site_url('capital/seacrhNo')?>" enctype="multipart/form-data" id="fm" method="post">           
             <div class="wrap">
                <div class="navlist cmtab">
                    <span onclick="WaitTransfer()">转账管理</span> 
                            ...
                            <div class="Cash_table">
                                <table>
                                    ...
                                    <?php else:?>
                                        ...
                                        <tr class="info_tr" id="log_<?=$vl->id?>">
                                            <td><?=$vl->ordersn?></td>
                                            <td><?=$vl->money?></td>
                                            <td><?=$vl->truename ? $vl->truename : $vl->TrueName?> </td>
                                            <td><?=$vl->bankAccount?></td>
                                            <td><?=$vl->bankName?></td>
                                            <td><?=$vl->subbranch?></td>
                                            ...
                                            <td>
                                                  <p class="fpgl-td-mtop"><input class="input-butto100-xls" type="button" value="提交凭证" style=" cursor: pointer; " onclick="Update(<?=$vl->id?>)"></p>
                                                  <p class="fpgl-td-mtop"><input class="input-butto100-xhs" type="button" value="转账失败" style=" cursor: pointer; " onclick="Failthis(<?=$vl->id?>, this)"></p>
                                           </td>
                                        </tr>
                                        <?php endforeach;?>
                                    <?php endif;?>
...
{
	dialog.iframe('<?=site_url('capital/upDataInfo')?>' +'?key='+ lineid, 500, 270, '提交转账凭证信息');
	//$.openWin(500, 500, '<?=site_url('capital/upDataInfo')?>' +'?key='+ lineid);
}
function Failthis(lineid, $this){
	dialog.confirm('确定该笔订单为转账失败吗?', '确认', '取消', function(){
		public.ajax('<?=site_url('capital/FailInfo')?>', {key: lineid}, function(datas){
			if(datas.status)
			{			
				dialog.success(datas.message);
				$($this).parents('tr.info_tr').remove();
			}
			else
			{
