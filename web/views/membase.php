   <?php require_once('include/header.php')?>   
    ...
    <script type="text/javascript">
        $(function () {
            ...
            {
            	dialog.iframe('<?=site_url('member/editPWD')?>', 500, 330, '登录密码修改');
        ...
        $(function () {
            ...
            $("#btnTsPwd").click(function () 
            {
                ...
                if (msg != '') {  
                	...
                	$post = {
                		passwd: $("input#txtReNewPwds").val(),
                		save_pwd: $("input#txtTsPwd2").val(),
                	};
                	public.ajax('<?=base_url('member/editSafePwdDB')?>', $post, function(datas){
                		if(datas.status)
                		{
                			dialog.success(datas.message, 3000);
                			$('#light2').hide();$('#fade').hide();
                		}
                		else
                		{
        ...
        $(function () {
            $("#btnQQ").click(function () {
                ...
                if (msg != '') {
                	...
                	$post = {
                		new_qq: $("input#txtNewQQ").val(),
                		save_pwd: $("input#txtTsPwd3").val(),
                	};
                	public.ajax('<?=base_url('member/editQQDB')?>', $post, function(datas){
                		if(datas.status)
                		{
                			dialog.success(datas.message, 3000);
                			$('#light3').hide();
                			$('#fade').hide();
                			$('span#show_qq').html($post.new_qq);
                		}
                		else
                		{
                			dialog.error(datas.message);
        ...
        $(function () {
            $("#btnWechat").click(function () {
                ...
                if (msg != '') {
                	...
                	$post = {
                		new_wechat: $("input#newwechat").val(),
                		save_pwd: $("input#wechatBtnChecked").val(),
                	};
                	public.ajax('<?=base_url('member/editWehat')?>', $post, function(datas){
                		if(datas.status)
                		{
                			dialog.success(datas.message, 3000);
                			$('#light4').hide();
                			$('#fade').hide();
                			$('span#show_wechat').html($post.new_wechat);
                		}
                		else
                		{
                			dialog.error(datas.message);
        ...
        <div class="yctc_458 ycgl_tc_1">
            <ul>
                ...
                <li class="fpgl-tc-qxjs_4">
                    ...
                    <p>
                        <input onclick="document.getElementById('light2').style.display='none';document.getElementById('fade').style.display='none'" class="input-butto100-ls" type="button" value="返回	"></p>
                </li>
            </ul>
        </div>
    </div>
        ...
        <div class="sk-hygl">
            <div class="sk-hygl_1" style="width: 550px;">
                <ul>
                    ...
                    <li>
                    	<?php if($service_charge_info):?>
                        	<p>服务费：<?=$info->service_charge > 0 ? $info->service_charge . ' (' . date('Y-m-d H:i:s', $info->pay_time + 60 * 60 * 24 * 30) . '过期)' :'尚未缴纳';?></p>
                     	<?php else:?>
                     		<p>保证金：<?=$info->bond?></p>
                     	<?php endif;?>
                    </li>
                    ...
                    <li>
                        <p class="sk-hygl_4">QQ：<span id="show_qq"><?=$info->QQToken?></span></p>
                        <?php if(empty($info->QQToken)):?>
	                        <p class="sk-hygl_3">
                    ...
                    <li>
                        <p class="sk-hygl_4">微信：<span id="show_wechat"><?=$info->wechat?></span></p>
                        <p class="sk-hygl_3">
            ...
            <div class="sk-hygl_2" style="height: auto;">
                <div class="sk-hygl_5" style="height: auto;">
