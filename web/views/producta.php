                                ...
                                <tr class="pro_list_tr">
                                    ...
                                    <td align="center" onclick=" ClickCheckBox('<?=$va->id?>') ">
                                         <a href="javascript:void(0)" title="<?=$va->status==1?'正常':'不正常'?>"><?=$va->status==1?'正常':'不正常'?>
                                            <?php if($va->status==0):?><span id="ms_<?=$va->id?>" style="color:red; font-size:12px; display:block;">请先设置好目标买手</span><?php endif;?>
                                            <?php if($va->peostatus!=1):?><span id="xw_<?=$va->id?>" style="color:red; font-size:12px; display:block;">请设置购买行为！</span><?php endif;?>
                                         </a>
                                    </td>
									<td>
                                    	<img width="150px" src="<?=$va->commodity_image;?>" alt="" />
                                    </td>
                                    <td >
                                         ...
                                         <a class="button-c thisbtn" onclick="delinfo(<?=$va->id?>, this)">删除</a>
...
<script>
    ...
    {
    	dialog.iframe("<?=site_url('member/sitepro')?>"+'?id='+proid, 800, 600, '目标客户设置');
//      $.openWin(580,860,"<?=site_url('member/sitepro')?>"+'?id='+proid);
    }
    function openwinout(proid){
    	dialog.iframe("<?=site_url('member/setpro')?>"+'?id='+proid, 800, 600, '买号购买行为');
    ...
    <script> 
function delinfo($id, $this){
	dialog.confirm('确认要删除所选商品信息吗?', '确认删除', '退出', function(){
		public.ajax('<?=site_url('member/delpro')?>', {id: $id}, function(datas){
			public.ajaxSuccess(datas, function(){
				$($this).parents('.pro_list_tr').remove();
			})
		});
	});
...
function ConfirmDelete()
{
	...
	if(aid != '')
	{
		dialog.confirm('确认批量删除所选商品信息吗?', '确认删除', '退出', function(){
			public.ajax('<?=site_url('member/delarr')?>', {pids: aid}, function(datas){
				public.ajaxSuccess(datas, function(){
					for(n=0;n<checks.length;n++)
				    {
						if(checks.eq(n).attr("value")!='on'){
							checks.eq(n).parents('.pro_list_tr').remove();
					  	}
					}
				});
			})
