...
<div class="yctc_458 ycgl_tc_1" style="height:100%; width:100%; overflow-y:auto">
            <ul> 
                    <li>
        ...
        reader.readAsDataURL(file[0]);
                ...
                <li class="fpgl-tc-qxjs_6">
                    <p style="width: 50%;">
                        ...
                        <input class="input-butto100-hs" type="button" id="btnSubmint" value="提交"  style="float: right; margin-right: 5px;">
                    </p>
                    <p style="width: 50%;">
...
<script type="text/javascript">
	$('input#btnSubmint').on('click', function(){
		...
		public.ajaxSubmit('fm', function(datas){
			if(datas.status)
			{
				var $shop_type = $shop_type ? '京东' : '淘宝';
					$shop_nature = $shop_nature == 1 ? '公司' : '个人',
					$sid = $('input[name=userid]').val(),
					$shop = $('#shop_' + $sid, window.parent.document);
				dialog.success(datas.message);
				$shop.find('.shop_name').html($shop_name);
				$shop.find('.shop_nature').html($shop_type + '<br/>(' + $shop_nature + ')');
				dialog.iframe_close();
			}
			else
			{
