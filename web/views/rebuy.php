...
<div class="fprw-pg" style="margin-top: 0px;">
    <table id="optionContainerShop">
        ...
        <tr>
            <td>
                <select class="select_108" id="IDSearchType0" name="SearchType[0]" data="SearchType" onchange="SearchTypeShopChange(this)">
                    <option value="1">淘宝APP自然搜索</option>
                    <option value="2">淘宝PC自然搜索</option>
                    <option value="3">淘宝APP淘口令</option>
                    <option value="4">淘宝APP直通车</option>
            ...
            <td>
                <input type="text" name="SearchKey[0]" id='IDSearchKey0' data="SearchKey" class="input_300" maxlength="500" value="" placeholder="请设置关键字">
            </td>
            <td>
                <input type="text" name="KeyWordCount[0]" id='IDKeyWordCount0' data="KeyWordCount" class="input_60" maxlength="3" placeholder="0" value="0" onkeyup="value=value.replace(/[^0-9]/g,'')" onchange="reTaskShopCount()">
            </td>            
            <td>
                <select id="IDRebuyShopPeriodType0" class="select_108" name="RebuyPeriodType[0]" data="RebuyPeriodType" style="width:158px;">
                    <option value="2">30天以上，50天以内</option>
                </select>
            </td>
            <td>
                <?=isset($counttaskshop)?$counttaskshop:0?>
            </td>
              <td>
    ...
    <script>
        ...
        function addShopRow() {
            ...
            if (seartypeNum > 10) {
                ...
                aDlist = aDlist.replace('RebuyPeriodType[]', "RebuyPeriodType["+rowCount+"]");
...
function CheckNext(){
	...
	if($("input[name=proid]").val()){
		...
		var n=0;
		$("#optionContainerShop input[class=input_300]").each(function () { /// 只是循环了关键字的
        ...
        var count = "<?=$counttaskshop?>";
        var want = $("#taskShopCount").html();
        if(want==0){
            $.openAlter("任务数量不能为0", "提示");
        }else{
            if(count>=want){
                    $("#frm").submit();
            }else{
