...
//			$dbsl ['shopid'] = $usertask->shopid;
	...
	public function CreateTaskError() {
		...
		if ($key == 0) {
            ...
            $sql = "SELECT COUNT(1) as has FROM zxjy_schedual WHERE usertaskid = {$key}";
            if ($master -> query($sql) -> row() -> has < 2){
                $usertask = $this->usertask->getInfo ( $key );
                $taskmodel = $this->task->getInfoModel ( $usertask->taskmodelid );
                if ($usertask != null) {
                ...
                echo '<script>alert("工单申请次数超出范围");history.back();</script>';
            }
		}
	}
...
//        var_dump($this -> db -> insert_id());
	...
	public function CreateTaskErrorDB()
    {
        ...
        if (isset($_POST ['TaskID']))
        {
            $_POST ['Category1ID']=='46'?$type = ' AND typeid = 46 ':$type = ' AND typeid != 46 ';
            $_POST ['Category1ID']=='46'?$messges = '该任务只能提交一次赔付工单 ':$messges = ' 该任务只能提交一次普通工单 ';
            $sql = "SELECT COUNT(1) as has FROM zxjy_schedual WHERE usertaskid = {$_POST['TaskID']}" . $type;
            if (!$master -> query($sql) -> row() -> has)  //无该笔任务工单记录
            //if (1)  //无该笔任务工单记录
            {
                $contentimg =  isset($_FILES ['multiple'])?$_POST ['Category1ID']=='46' && $_POST ['Category2ID'] !='49'?$this -> uploadFilePF($_FILES ['multiple']) :$this -> uploadFile($_FILES ['multiple']) : '';  //赔付工单除了恶意退款以外都要上传excel表
                $_POST ['Category1ID']=='46'?(!empty($contentimg) && !empty($_POST['PFmoneycount'])?'':show(0, '证据提交有误，请稍后再试')):'';
				$res = true;
                ...
                $db = [
                    ...
                    'checktime' =>$_POST ['Category1ID']=='46'? strtotime($_POST['CheckDate']):null,//赔付工单有被查时间
                    'checknumber' =>$_POST ['Category1ID']=='46'? $_POST['CheckNum']:null,//赔付工单有被查单数
                    'pfmoney' =>$_POST ['Category1ID']=='46'? $_POST['PFmoneycount']:null,//赔付工单有被查单数
                    'merchantid' => $usertask -> merchantid,
                    'buyerid' =>$_POST ['Category1ID']=='46'? '0': $usertask -> userid,//赔付工单buyerid为0
...
//		$re ? show(1, '工单信息已经提交到后台,我们的工作人员会尽快处理') : show(0, '工单插入数据库失败，请稍后再试');
	...
	public function  DownloadDemo() {
        ...
        $data = file_get_contents("./download/PFdemo.xls");
        $name = 'PFdemo.xls';
    ...
    public function addEvaluateDB()
    {
        ...
        if (isset($_POST ['id']))
        {
            ...
            if (!empty($shop_infos -> seller_userid) && $db['use_helper'] -> value != 0)  //如果有订购小助手并且尚未过期，判断订单信息以及状态是否符合
            {
                $curl_args = [
                    'tid' => $usertaskinfo -> ordersn,
                    'fields' => 'status,consign_time',
                ];
                $curl_exe = $this -> curlApi($shop_infos -> seller_userid, $curl_args, 'getTradeFull');
                extract($curl_exe);
                if (!$curl_error)  //curl无错误
                {
                    if ($tmpInfo -> code == 1)  //成功获取订单信息
                    {
                        if (in_array($tmpInfo -> data -> status, ['WAIT_BUYER_CONFIRM_GOODS', 'TRADE_BUYER_SIGNED', 'TRADE_FINISHED']))  //订单状态为[卖家已发货]
                        {
                            ...
                            {
                                $distance_helper = time() - strtotime($tmpInfo -> data -> consignTime);
                                if ($tmpInfo -> data -> status == 'WAIT_BUYER_CONFIRM_GOODS' && $distance_helper < $limit)  //订单状态为[卖家已发货]且发货时间小于80小时
                                {
                                    show(0, '当前订单状态为[商家已发货，等待买家确认]，发货时间为' . $tmpInfo -> data -> consignTime . '，需' . $this -> time2second($limit - $distance_helper) . '之后才可发布评价任务');
                                }
                            }
                            else
