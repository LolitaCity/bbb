	...
	public function republish()
    {
        if (isset($_GET['taskid']))
        {
            $sql = 'SELECT p.commodity_id, p.commodity_abbreviation, p.commodity_title, p.commodity_url, s.shopname, t.intlet, t.keyword, t.number, tm.price, tm.auction, tm.express, 
                      ...
                      tm.how_search, tm.new_keyword, t.tasktype, t.order, t.price as other_price, t.sendaddress, t.other FROM zxjy_task as t
            ...
            if ($new_taskid = $this->republish_handler('task', $task, $task_diff_data))
            {
                ...
                if ($new_taskmodelid = $this->republish_handler('taskmodel', $taskmodel, $taskmodel_diff_data))
                {
                    ...
                    $tasktime_diff_data = [
                        ...
                        'takenumber' => 0,
                        'del' => 0,
						'beginPay' => strtotime(date('Y-m-d', strtotime($average_time[0]) + 60 * 60 * 24) . ' 09:00:00'),
                        'EndPay' => strtotime(date('Y-m-d', strtotime($average_time[0]) + 60 * 60 * 24) . ' 23:59:59'),
                    ];
...
//		echo json_encode ( $json );
	...
	public function taskyes() {
        ...
        //未接任务sql8
	    $sql = 'SELECT FROM_UNIXTIME(tt.beginPay) as beginPay, FROM_UNIXTIME(tt.endPay) as endPay, ut.id, ut.tasksn, ut.ordersn, ut.status, ut.addtime, ut.expressnum, ut.userid, ut.taskid, ut.complaint, ut.showpicbtn, t.addtime as typeaddtime, t.tasktype, ww.wangwang, s.shopname, tm.price, tm.express, tm.commission from zxjy_usertask as ut 
	    ...
	    $sql = 'SELECT FROM_UNIXTIME(tt.beginPay) as beginPay, FROM_UNIXTIME(tt.endPay) as endPay, ut.handsel, ut.browseimg, ut.tasktwoimg, ut.id, ut.tasksn, ut.taskid, ut.ordersn, ut.status, ut.addtime, ut.userid, t.addtime as typeaddtime, t.tasktype, ww.wangwang, s.shopname, tm.price, tm.express, tm.commission from zxjy_pretask as ut 
                LEFT JOIN zxjy_task as t on ut.taskid = t.id
                LEFT JOIN zxjy_tasktime as tt on tt.pid = t.mark
                LEFT JOIN zxjy_taskmodel as tm on tm.id = ut.taskmodelid
                LEFT JOIN zxjy_blindwangwang as ww on ww.userid = ut.userid
    ...
    public function searchtask()
    {
        ...
        $condition .= !empty($_GET['BeginDate']) ? ' AND t.addtime > ' . strtotime($BeginDate) : '';
        $condition .= !empty($_GET['EndDate']) ? ' AND t.addtime < ' . strtotime($EndDate) : '';
        $sql = 'SELECT FROM_UNIXTIME(tt.beginPay) as beginPay, FROM_UNIXTIME(tt.endPay) as endPay, ut.handsel, ut.browseimg, ut.tasktwoimg, ut.id, ut.tasksn, ut.ordersn, ut.status, ut.addtime, ut.userid, t.addtime as typeaddtime, t.tasktype, ww.wangwang, s.shopname, tm.price, tm.express, tm.commission from zxjy_pretask as ut 
                LEFT JOIN zxjy_task as t on ut.taskid = t.id
                LEFT JOIN zxjy_tasktime as tt on tt.pid = t.mark
                LEFT JOIN zxjy_taskmodel as tm on tm.id = ut.taskmodelid
                LEFT JOIN zxjy_blindwangwang as ww on ww.userid = ut.userid
        ...
        $db ['status'] = $status = @$_GET ['status'] == 0 ? 'all' : @$_GET ['status'];
...
//		}
    ...
    public function checkPayPasswd()
    {
        if (isset($_POST ['TradersPassword']))
        {
            ...
            $sql = 'SELECT SUM(if(t.number > 0, t.number, 0)) as has FROM zxjy_task as t 
                    LEFT JOIN zxjy_tasktime as tt on tt.pid = t.mark
                    WHERE t.userid = ' . $this -> uid . ' AND TO_DAYS(FROM_UNIXTIME(t.addtime)) = TO_DAYS(NOW()) AND t.del = 0 AND tt.close >= UNIX_TIMESTAMP()';
            ...
            if ($db['info'] -> maxtask - $has_release >= $number || $TaskPlanType == 2)  //多天平均任务不做判断
            {
                ...
                $sum_field = $return_cash_way ? 'commission + tm.price * auction + express' : 'commission';
                ...
                $has_no_pay = $this -> db -> query($sql) -> first_row() -> no_pay;
                $sql = 'SELECT value FROM zxjy_system WHERE varname = "cash_deposit"';
                ...
                if ($db['info'] -> Money - $cash_deposit - $has_no_pay >= 0)
                {
                    ...
                    show(3, '当前余额(' . $db['info'] -> Money . ')不足以支付剩余有效任务的费用(' . $has_no_pay . ')，请先充值再发布');
            }
            else
                show(0, '超出今日单量限制, 请检查任务情况');
        }
        else
            show(0, '参数异常, 请刷新页面再试');
    }
    ...
    {
        if (isset($_POST ['AddToPoint']) && isset($_POST ['remark']))
        {
	...
	public function CancelOne()
    {
        if (isset($_POST['key']))
        {
            ...
            if ($info && $usertaskinfo)
            {
                ...
                $db ['doing'] = 3;
                $res = $res && $this -> taskevaluate -> updata($info->id, $db);
                // 取消评价任务返还给用户已扣空金额
...
//				// 取消评价任务返还给用户已扣空金额
//				$id = $this->session->userdata ( 'sellerid' );
//				$seller = $this->user->getInfo ( $id );
//				$dbuser ['Money'] = $seller->Money + $info->iscommission;
//
//				$this->user->updata ( $seller->id, $dbuser ); // 还钱成功
//
//				$this->load->model ( 'Cash_model', 'cash' );
//				$dbcash ['type'] = '取消评价任务';
//				$dbcash ['remoney'] = $seller->Money;
//				$dbcash ['increase'] = '+' . $info->iscommission;
//				$dbcash ['beizhu'] = '取消评价任务返还佣金';
//				$dbcash ['addtime'] = time ();
//				$dbcash ['userid'] = $seller->id;
...
//        var_dump($this -> db -> insert_id());
	...
	public function CreateTaskErrorDB()
    {
        if (isset($_POST ['TaskID']))
        {
            if (!$this -> complaint -> geyInfoByUsertaskID($_POST ['TaskID']))  //无该笔任务工单记录
            {
